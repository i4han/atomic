...
create = (new_site) ->
    new_site = 'a'
    console.log new_site, 'not yet finished.'
    mkdir add(apps_path, new_site), ->
        cd add apps_path, new_site
        meteor_command 'create', 'app'
        cd app apps_path, new_site, 'app'
        meteor_packages_removed.map (p) -> meteor_command 'remove', p
        meteor_packages.map (p) -> meteor_command 'add', p
