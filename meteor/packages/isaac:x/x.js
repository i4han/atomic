...
var NotPxDefaultProperties, indent_string, keyFix, tideEventKey, tideKey, validateRe,
...
x.toDash = function(str) {
  ...
  return str.replace(/\$([0-9])/g, function($1) {
    return '-' + $1.slice(1);
  });
};

x.toObject = function(a) {
  var v;
...
x.interpolateObj = function(o, data) {
...
x.interpolateOO = function(options, data) {
...
x.addProperty = function(obj, key, value) {
  obj[key] = value;
  return obj;
};
...
x.indentStyle = function(obj, depth) {
  if (depth == null) {
    depth = 1;
  }
  if (!x.isObject(obj)) {
...
x.indent = function(b, i) {
  if (i) {
    return b.replace(/^/gm, Array(i + 1).join(indent_string));
  } else {
    return b;
  }
};
...
x.tideKey = function(obj, fid, seperator) {
  if (!x.isObject(obj)) {
    return obj;
  }
  return x.keys(obj).reduce((function(o, k) {
    ...
    o[tideKey(k, fid, seperator)] = (function() {
      switch (false) {
        case !x.isObject(ok = obj[k]):
          return x.tideKey(ok, fid, seperator);
        default:
          return ok;
      }
    })();
...
x.tideEventKey = function(obj, fid) {
  return x.keys(obj).reduce((function(o, k) {
    return x.addProperty(o, tideEventKey(k, fid), obj[k]);
  }), {});
};

x.tideValue = function(obj) {
  if (!x.isObject(obj)) {
    return obj;
  }
  return x.keys(obj).reduce((function(o, k) {
...
x.Module = (function() {
  ...
  Module.prototype.id = function(str) {
    var _this = this;

    if (str.indexOf(' ') > -1) {
      return str.split(' ').map(function(s) {
        return '#' + window.Module[_this.name].block + '-' + _this.name + '-' + s;
      });
    } else {
      return '#' + window.Module[this.name].block + '-' + this.name + '-' + str;
    }
  };

  Module.prototype._instance = function(i) {
